...
public class ToastUtils {
    ...
    public static void show(CharSequence text, int duration) {
        ...
        if (System.currentTimeMillis() - sTime > intervals) {
            try {
                Toast toast = Toast.makeText(AppManager.getInstance()
                        .getApplication().getApplicationContext(), text, duration);
    ...
    public static void show(int resId, int duration) {
        show(AppManager.getInstance().getApplication()
                .getApplicationContext().getString(resId), duration);
    }
