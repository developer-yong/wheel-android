package dev.yong.photo.view.photoview;
...
public class PhotoView extends androidx.appcompat.widget.AppCompatImageView {
    ...
    private int maxAnimFromWaite = 500;
    ...
    private void init() {
        ...
        maxFlingOverScroll = (int) (density * 30);
        maxOverResistance = (int) (density * 140);
    ...
    private boolean hasSize(Drawable d) {
        return (d.getIntrinsicWidth() > 0 && d.getIntrinsicHeight() > 0)
    ...
    private static int getDrawableWidth(Drawable d) {
        return d.getIntrinsicWidth() > 0 ? d.getIntrinsicWidth()
                : d.getMinimumWidth() > 0 ? d.getMinimumWidth() : d.getBounds().width();
    }

    private static int getDrawableHeight(Drawable d) {
        return d.getIntrinsicHeight() > 0 ? d.getIntrinsicHeight()
                : d.getMinimumHeight() > 0 ? d.getMinimumHeight() : d.getBounds().height();
    }
...
//            mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
    ...
    private float resistanceScrollByX(float overScroll, float detalX) {
        return detalX * (Math.abs(Math.abs(overScroll) - maxOverResistance) / (float) maxOverResistance);
    }

    private float resistanceScrollByY(float overScroll, float detalY) {
        return detalY * (Math.abs(Math.abs(overScroll) - maxOverResistance) / (float) maxOverResistance);
    }
    ...
    private class Transform implements Runnable {
        ...
        void withFling(float velocityX, float velocityY) {
            ...
            mFlingScroller.fling(mLastFlingX, mLastFlingY, (int) velocityX, (int) velocityY, minX, maxX, minY, maxY, Math.abs(overX) < maxFlingOverScroll * 2 ? 0 : maxFlingOverScroll, Math.abs(overY) < maxFlingOverScroll * 2 ? 0 : maxFlingOverScroll);
        }
     ...
     */
    public void animFrom(Info info) {
        if (isInit) {
            reset();
    ...
    public void animTo(Info info, Runnable completeCallBack) {
        if (isInit) {
            mTranslate.stop();
